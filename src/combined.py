...
# geobind modules
from nn.layers import ContinuousCRF
from eq_conv import EQConv
from nn.utils import MLP
...
class SAModule(torch.nn.Module):
    def __init__(self, nIn, nOut, conv_args, ratio, radius,
        ...
        if conv_args['name'] == 'PointConv':
            ...
            dim = nIn + 4
            nn1 = MLP([dim, dim, dim], batch_norm=False)
            nn2 = MLP([dim, nOut], batch_norm=False)
            self.conv = PPFConv(nn1, nn2, add_self_loops=False)
            self.conv.aggr = conv_args.get('aggr', 'max')
        elif conv_args['name'] == "EQConv":
            irreps_in = str(nIn) + "x0e + 1x1o"
            irreps_out = str(nOut) + "x0e + 1x1o"
            self.conv = EQConv(irreps_in, irreps_out)
    ...
    def forward(self, x, pos, batch, norm=None):
        ...
        if self.conv_name == 'PointConv':
            ...
            #print(edge_src, edge_src.size())
            
            f_in = torch.cat((x,pos),dim=1)
            x = self.conv(f_in, pos, edge_src, edge_dst, self.r)[idx][:,:self.nOut]
...
class FPModule(torch.nn.Module):
    def __init__(self, nIn, nSkip, nOut, k=3, batch_norm=False, bn_kwargs={}):
...
class PointNetPPBinding(torch.nn.Module):
    def __init__(self, nIn, nOut=None, 
        ...
        ### Model Layers ###
        # linear input
        self.lin_in = MLP([nIn, nhidden, nhidden], batch_norm=False)
        ...
        self.lin_final = MLP([nhidden*2,nhidden,nhidden//2,1],act=['elu', 'elu', None],batch_norm=[False,False,False,False])
    ...
    def forward(self, data):
        ...
        #if self.use_lin:
            #x = F.dropout(x, p=self.v_dropout, training=self.training)
        x_cls = self.lin_out(x[data.protein_mask])
        #print(x.size(), "e")
        # crf layer
        if self.use_crf:
            x_cls = self.crf1(x_cls, data.edge_index)
        #print(x ,"f")
        """
